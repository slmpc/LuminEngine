...
namespace lumin::render::runtime {
    ...
    /** Pipeline session 向同步 Renderer 发布的只读状态。 */
    ...
    struct RenderPipelineSessionCreateContext {
        /** 渲染主线程独占且由 `Renderer` 拥有的 Vulkan/NvRHI context。 */
        VulkanContext* vulkan = nullptr;
        /** 完全拥有的初始不可变世界快照。 */
        world::RenderWorldSnapshotPtr initialWorld;
        /** 编译后 shader 目录。 */
        std::filesystem::path shaderDirectory;
        /** 渲染主线程拥有且覆盖 session 创建过程的字体图集。 */
        ImFontAtlas* uiFontAtlas = nullptr;
    };

    /**
     * @brief 由渲染主线程独占的一条可执行 Pipeline session。
     *
    ...
    public:
         ...
         * @return GPU submit
         * 与随后的交换链 present 流程完成时返回 `true`；最小化或 acquire 重试返回 `false`。
         * @thread_safety
         * 只能由拥有 session 的渲染主线程调用。
         */
        [[nodiscard]] virtual bool drawFrame(core::RenderFramePacket packet, const ImDrawData& ui) = 0;
    ...
    public:
        ...
        /**
         * @brief 在当前渲染主线程创建完整候选 session。
         * @param context
         * 被消费的显式创建输入；工厂不得读取活动 Level、Camera、Editor 或 ImGui 状态。
