    ...
    namespace {
        ...
        public:
                    ...
                    if (creationDriver_ != nullptr) {
                        for (FrameResources& frame : frames_) {
                            ...
                            if (!frame.uniformBuffer) {
                                throw std::runtime_error("Failed to create an SSAO uniform buffer.");
                            }
                        }
                    }
#endif
            ...
            void addPasses(FrameGraph& frameGraph, const FrameInfo& frameInfo) override {
                ...
                if (frameInfo.extent.width == 0 || frameInfo.extent.height == 0) {
                    throw std::invalid_argument("SSAO pass requires a non-empty render extent.");
                }
...
#if defined(LUMIN_GI_TESTING)
                SsaoRecordProbe* const recordProbe = recordProbe_;
#endif

                frameGraph.addPass(
                    "GI: SSAO clear", FrameGraphPassType::Transfer,
                    ...
                    [output
                     ...
                     recordProbe
                        ...
                        if (recordProbe != nullptr) {
                            detail::recordSsaoClear(*recordProbe, output);
                            return;
                        }
#endif
                        if (context.commandList == nullptr) {
                            throw std::logic_error("SSAO clear pass requires an NvRHI command list.");
                        }
                    ...
                    [output, bindingSet, framebuffer, pipeline, extent
                     ...
                     recordProbe
                        ...
                        if (recordProbe != nullptr) {
                            ...
                            detail::recordSsaoFullscreen(*recordProbe, state);
                            return;
                        }
#endif
                        if (context.commandList == nullptr) {
                            throw std::logic_error("SSAO pass requires an NvRHI command list.");
                        }
                        nvrhi::GraphicsState state;
                        state.setPipeline(pipeline)
                            .setFramebuffer(framebuffer)
                            ...
                            .addBindingSet(bindingSet);
                        detail::recordSsaoFullscreen(*context.commandList, state);
                    });
            }
        ...
        private:
            void createBindings(const nvrhi::SamplerHandle& sampler) {
                ...
                if (!bindingLayout_) {
                    throw std::runtime_error("Failed to create the SSAO binding layout.");
                }
                ...
                for (const FrameResources& frame : frames_) {
                    ...
                    if (!bindingSet) {
                        throw std::runtime_error("Failed to create an SSAO binding set.");
                    }
            ...
            void createFramebuffers() {
                ...
                for (const FrameResources& frame : frames_) {
                    ...
                    if (creationDriver_ != nullptr) {
                        ...
                        if (!framebuffer) {
                            throw std::runtime_error("Failed to create an SSAO framebuffer.");
                        }
            ...
            void createPipeline(nvrhi::Format outputFormat) {
                ...
                const nvrhi::ShaderHandle vertexShader =
                    shaders_->loadModule("ssao.vert.spv", nvrhi::ShaderType::Vertex, "vertexMain");
                const nvrhi::ShaderHandle fragmentShader =
                    shaders_->loadModule("ssao.frag.spv", nvrhi::ShaderType::Pixel, "fragmentMain");
...
#if defined(LUMIN_GI_TESTING)
            SsaoCreationDriver* creationDriver_ = nullptr;
            SsaoRecordProbe* recordProbe_ = nullptr;
#endif
        };
