...
namespace lumin::render::gi {
    ...
    struct HybridLightingCompositePass::Impl {
        ...
        explicit Impl(const HybridLightingCompositeCreateInfo& createInfo)
            ...
            const nvrhi::ShaderHandle shader =
                shaders.loadComputeModule("hybrid_lighting_composite.comp.spv", "compositeMain");
            const std::array layouts = {bindingLayout};
            pipeline = pipelines.createComputePipeline(ComputePipelineDesc{.computeShader = shader,
    ...
    HybridLightingCompositePass::HybridLightingCompositePass(
        const HybridLightingCompositeCreateInfo& createInfo) {
        if (createInfo.device == nullptr || createInfo.shaderDirectory.empty() || createInfo.extent.isEmpty() ||
            createInfo.frameSlotCount == 0) {
            throw std::invalid_argument("Hybrid lighting composite requires device, extent, shader directory, and slots.");
        }
    ...
    FrameGraphPassHandle HybridLightingCompositePass::record(
        ...
        const FrameGraphResourceHandle constantsResource = frameGraph.importBuffer(
            "hybrid-lighting-composite.constants." + std::to_string(frameSlot),
