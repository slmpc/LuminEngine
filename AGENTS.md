...
- 为并发、资源生命周期、同步、状态事务、能力降级和其他关键实现添加简洁的中文注释；注释应解释约束与原因，
  不要逐句复述代码。
- 所有公开类型、函数、枚举和重要字段都必须使用中文 Doxygen 注释，风格类似 JavaDoc；至少说明用途、所有权或
  生命周期约束，并按需补充参数、返回值、异常与线程要求。
...
- 文档必须与当前源码、构建 target、配置项和实际运行行为保持一致；修改公开接口、模块依赖、资源所有权、帧顺序或
  降级策略时，必须在同一变更中更新对应文档，禁止保留已经失效的设计描述。
